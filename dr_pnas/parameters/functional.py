...
from dr_pnas.utils import bearing_height, area_above, func_shift, find_decline, trap_rule
from dr_pnas.parameters.amplitude import S_q
...
def S_vk(channel: np.array) -> float:
    ...
    # this if operation is added by Mik so the code won't crash with small patches 
    if len(np.where(bin_edges <= right_height)[0])==0:
        height = 0
    else:       

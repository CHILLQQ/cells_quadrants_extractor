...
from dr_pnas.utils import (find_summits,
                      find_local_maxs,
...
from dr_pnas.parameters.amplitude import S_z
...
def S_ds(channel: np.array,
    ...
    #print("The amount of summits is ",len(summits))
...
def S_cl(spectrum: np.array,
    ...
    # We normalize the correlation length w.r.t. distance of zero lag idx to the edge
    return np.min(radii) / np.sqrt((x0 * dx) ** 2 + (y0 * dy) ** 2)
